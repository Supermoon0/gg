...
## 실네트워크 재실증 (같은 날 오후 — egress 허용 후)

허용목록이 열린 뒤에도 엔진은 여전히 차단됐는데, 원인은 net.py가 프록시
없이 직접 소켓을 쓰기 때문(허용은 프록시 계층에만 적용). **net.py에 표준
환경 프록시 지원 추가**: https는 `HTTPS_PROXY`로 CONNECT 터널 후 TLS,
http는 절대 URI, `NO_PROXY` 존중(로컬 건틀릿은 그대로 직접 소켓 —
net_gauntlet 14/14 회귀 무사).

- **smoke 전수 실측**: 실네트워크 example.com 페치까지 **148/149 통과**.
  남은 1건(google plain-http 리다이렉트)은 환경이 https 전용이라 불가
  (http_proxy 부재 + 직접 소켓 차단 — curl도 동일하게 403).
- **basket 실사이트 스코어보드 — 7/9 "읽을만함", 크래시 0**:

  | 사이트 | 노드 | 페인트 | 텍스트 | JS오류 | 시간 | 결과 |
  |---|---|---|---|---|---|---|
  | 네이버 | 195 | 567 | **546** | 5 | 3.2s | 읽을만함 |
  | 위키백과 | 3908 | 4581 | 3796 | 0 | 1.5s | 읽을만함 |
  | 나무위키 | 18 | 0 | 0 | 0 | 0.5s | 실패* |
  | 연합뉴스 | 2869 | 2909 | 1324 | 13 | 6.9s | 읽을만함 |
  | 티스토리 | 419 | 326 | 112 | 2 | 5.4s | 읽을만함 |
  | HN | 818 | 872 | 746 | 0 | 1.5s | 읽을만함 |
  | MDN | 920 | 1332 | 1068 | 4 | 1.3s | 읽을만함 |
  | example | 12 | 20 | 19 | 0 | 0.5s | 빈약** |
  | 정부24 | 1154 | 119 | 65 | 5 | 8.4s | 읽을만함 |

  \* 나무위키는 Cloudflare 봇월("Just a moment..." 챌린지 5.7KB)을 서빙 —
  엔진 아닌 서버측 차단. \*\* example.com은 문단 1개짜리 페이지라
  텍스트>40 기준을 원리적으로 못 넘음(스코어 기준의 한계).
- **네이버 시각 증거**(`validation/out/evidence_naver.png`): HTML 216KB
  실페치 → 초록 알약 검색창 + 회색 placeholder(크롬 JS-off 외형) +
  리더 모드 섹션에 당일 연합뉴스 헤드라인 10건·관심사 피드 실렌더.
  679 페인트/658 텍스트, 문서 4176px, 웜 2.4s. JS 콘솔: NBP_CORP
  미정의(문서화된 외부 SDK 스킵 범주)·"Cannot call a class as a
  function" 1건·서로게이트 정규식 미지원 경고.
