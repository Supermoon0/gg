...
## 실사이트 바스켓 소견 (2026-07-25, 첫 CI 실행)

- 네이버: "실패" (218 노드/28 페인트, JS 오류 0) — GitHub 러너와 로컬
  컨테이너에서 동일 재현. DOM이 비정상적으로 작아 데이터센터 IP에 대한
  경량/차단 응답이거나 초기 번들 경로 회귀로 보인다. **다음 세션 조사
  후보**: 응답 본문을 덤프해 어떤 문서를 받는지부터 확인할 것.
- 나무위키 측정 중 hosted 러너가 shutdown signal(143)로 종료 — 자원
  고갈 의심. 바스켓 workflow에 사이트별 타임아웃/메모리 가드 필요.
- 위키백과는 CI에서도 "읽을만함" 정상.
