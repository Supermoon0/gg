...
| CSS (render) | **6,336/17,263 (36.70%)** | WPT css reftest, 픽셀 비교 |
...
#### 궤적: 28.25% → 27.47% → 30.01% → 31.53% → 33.50% → 35.22% → 35.49% → 36.23% → 36.34% → 36.46% → 36.56% → 36.59% → 36.66% → 36.70%
...
- [x] **`opacity`** — `opacity: 0.05` 미만 컬링 한 곳에서만 읽히고
      ...
      *주의: 그룹 합성이 아니라 명령별 알파다. 페이드된 서브트리 안에서
      ...
      전용 오프스크린 버퍼가 필요하다*
- [x] **`order`** — flex/grid 항목이 전부 소스 순서로 배치됐다. 마크업은
      사이드바를 뒤에 두고 화면에서는 앞에 두는 반응형 레이아웃이 전부
      모바일 순서로 렌더되고 있었다. 정렬은 stable, 미지정은 0
      (grid-items 11/60 → 16/60)
- [x] **`clip-path: inset()` · `filter: opacity()`** — inset은 직사각형이라
      기존 클립 명령에 그대로 맞는다. circle/ellipse/polygon은 바운딩
...
**가짜 통과를 지키려고 옳은 구현을 미루지 않는다.** opacity와
`clip-path: inset()`은 둘 다 해당 영역 점수를 *내렸다*. 이유가 같다 —
이전엔 테스트와 레퍼런스가 **양쪽 다 아무것도 안 그려서** 일치했고,
이제 레퍼런스만 정확히 그려지면서 어긋난다. 그 테스트들은 애니메이션을
비교 시점까지 돌려야 통과하는데 이 하네스는 스크립트를 안 돌린다 —
**어느 쪽이든 통과할 수 없었던 테스트**다. 내려간 점수는 정보이지 회귀가
아니고, 그 구분을 커밋 메시지에 매번 적었다.
