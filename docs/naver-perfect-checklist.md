...
| 단계 | 기준 | 필요한 마일스톤 | 상태 (07-27) |
|---|---|---|---|
| **A. 크롬 JS-off 동급** | 로고·둥근 검색창·아이콘이 픽셀 수준으로 같음 | M1 | **사실상 달성** (M1 주요 항목 완료, 웹폰트·flex 심화 등 소품 잔여) |
| **B. 첫 화면 완성** | 뉴스·쇼핑·피드가 실제로 그려짐 | M2 + M3 + M4 | **07-27: 광범위 달성** — 뉴스스탠드·언론사 탭·**광고 3종(타임보드·우측 레일·인피드) 실제 크리에이티브**·recoshopping 상품 캐러셀·날씨/증시/환율/캘린더/VIBE 위젯·추천 피드 3섹션·해피빈. 노드 ~1400, 이미지 ~55, **JS 오류 0**. 잔여: **shopsquare 좌측 그리드**(hydrate까지 완주, AppProvider 세그먼트 병합에서 정지 — docs/shopsquare-hydration-stall.md) |
| **C. 사용 가능** | 스크롤·호버·검색 타이핑·클릭 이동 | M5 | 스크롤·타이핑·GET 제출·**hover/focus 재스타일**(07-18) · 잔여: IME |
| **D. 크롬급 속도** | 재방문 1초 내 첫 화면, 스크롤 60fps | M6 | 재방문 0.18s ✅ · settle ~9s(초기 로드) · 60fps 미측정 |

## 07-27 세션 종합 — "네이버 100%" 마무리 시도

1번(shopsquare)~3번(실행 변동)을 끝까지 추적한 확정 결과:

- **#1 shopsquare 좌측 그리드** — 이번 세션 다수 엔진 수정(Promise
  생성자화, 동적 스크립트 서비스, `el.attributes` 라이브 맵,
  getElementsByName/attachShadow/crypto, 힙 회계, 마이크로태스크
  드레인 오프바이원)으로 **부트가 hydrate까지 완주**. 최종 정지점은
  React 19 app-router의 AppProvider(layout) 세그먼트가 렌더 트리에
  병합되지 않아 `useAppState`가 undefined 컨텍스트를 읽는 지점 —
  엔진이 아니라 세그먼트-머지 내부. 전용 세션 대상으로 문서화.
- **#2 피드 텍스트 겹침** — render_audit이 특정 실행에서 잡은 7건은
  **재현되지 않음**(3연속 overlaps=0). y<0의 blind/sr-only 숨김
  텍스트이거나 특정 실행에만 로드된 웹툰 항목. 현시점 실재 결함 아님.
- **#3 실행별 변동** — **엔진 결함 아님**으로 확정. 동일 바이트를
  고정해 3회 로드 시 DOM이 **완전히 동일**(els=1081, 동일 시그니처).
  변동의 원인은 서버의 매 요청 광고 로테이션·피드 셔플로, 실제
  브라우저도 동일하게 매번 다르다. `determinism.py`로 검증.

결론: B단계의 남은 한 조각은 shopsquare 하나이며, 그것은 우리 엔진의
결함이 아니라 React 19 세그먼트 병합의 정밀 추적을 요하는 별건이다.
