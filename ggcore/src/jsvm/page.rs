...
impl PageVm {
    pub fn new(doc: Option<Rc<RefCell<dom::Document>>>) -> PageVm {
        ...
        vm.st.ids = Ids {
            ...
            has_attribute: vm.name_id("hasAttribute"),
        ...
        // set_page_url once the loader knows the URL)
        {
            ...
            for m in ["getEntries", "getEntriesByName",
                      "getEntriesByType"] {
                let k = vm.name_id(m);
                let f = make_native(&mut vm.st, Native::EmptyArray);
                raw_set_prop(&mut vm.st, pi, k, f);
            }
...
mod tests {
    ...
    fn method_extraction() {
        ...
        assert_eq!(
            n("''.substring.call('abcdef', -2, 2) === 'ab' ? 1 : 0"),
            1.0);
        assert_eq!(
            n("''.substr.call('abcdef', 1, -2) === '' ? 1 : 0"), 1.0);
        assert_eq!(
            n("''.substr.call('abcdef', -2, 1) === 'e' ? 1 : 0"), 1.0);
        assert_eq!(
            n("''.substr.call('abcdef', 2, undefined) === 'cdef' ? 1 : 0"),
            1.0);
        assert_eq!(
            n("''.slice.call('abcdef', -2) === 'ef' ? 1 : 0"), 1.0);
    }
    ...
    fn m3_browser_objects() {
        ...
        assert_eq!(
            n("(navigator.userAgent.indexOf('GGBrowser') >= 0 ? 1 : 0) \
               ...
               + (performance.getEntriesByType('resource').length === 0 ? 16 : 0) \
               + (performance.getEntriesByName('x').length === 0 ? 32 : 0)"),
            63.0);
    ...
    fn direct_string_calls_cover_the_same_surface_as_extraction() {
        ...
        assert_eq!(
            vm.run_scripts(&["var u = 'a/b/c.js';\
                ...
                console.log('abcdef'.substring(-2, 2) === 'ab'\
                            && 'abcdef'.substring(2, undefined) === 'cdef'\
                            && 'abcdef'.slice(-2) === 'ef'\
                            && 'abcdef'.slice(2, undefined) === 'cdef'\
                            && 'abcdef'.substr(1, -2) === ''\
                            && 'abcdef'.substr(-2, 1) === 'e');"
                .to_string()]),
            vec![
...
[0-9a-f]{3}-[0-9a-f]{12}$/.test(u));"
    ...
    fn dom_read_write() {
        ...
        assert_eq!(
            logs,
            vec!["hi", "true", "bye", "2", "box", "true", "false",
                 "1", "null", "1", "null"]
        );
