//! Named character references, generated from the WHATWG
//! entities.json (the spec's own table). Sorted by name so the
//! tokenizer's longest-match search can binary-search prefixes.
//!
//! Names keep their trailing ';' where the spec requires one; the
//! legacy semicolon-less forms are separate entries, exactly as the
//! table defines them.
